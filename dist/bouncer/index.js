...
var cors2 = (options) => {
  ...
  return async function cors22(c2, next) {
...
app.post("/chat", async (c2) => {
  try {
    const { message, attempts = 1, sessionId = "default" } = await c2.req.json();
    if (!message) {
      return c2.json({ error: "Message is required" }, 400);
    }
    const session = getSession(sessionId);
    ...
    const response = await fetch("https://api.cerebras.ai/v1/chat/completions", {
      method: "POST",
      headers: {
        "Authorization": `Bearer ${c2.env.CEREBRAS_API_KEY}`,
        "Content-Type": "application/json"
      },
      body: JSON.stringify({
        model: "llama-3.3-70b",
...
app.post("/win", async (c2) => {
  try {
    const { username, attempts, method } = await c2.req.json();
    if (!username || typeof username !== "string") {
      return c2.json({ error: "Valid username is required" }, 400);
    }
    const cleanUsername = username.trim().slice(0, 20);
    if (cleanUsername.length < 1) {
      return c2.json({ error: "Username cannot be empty" }, 400);
    }
    const vipData = await getVIPList(c2.env);
    const exists = vipData.vips.some(
      (vip) => vip.username.toLowerCase() === cleanUsername.toLowerCase()
    );
    if (exists) {
      return c2.json({ error: "Username already on VIP list" }, 409);
    }
    ...
    await saveVIPList(c2.env, vipData);
    return c2.json({
      success: true,
    ...
    return c2.json({ error: "Failed to save to VIP list" }, 500);
  }
});
app.get("/leaderboard", async (c2) => {
  try {
    const vipData = await getVIPList(c2.env);
    return c2.json(vipData);
  } catch (error) {
    return c2.json({ error: "Failed to get leaderboard" }, 500);
  }
});
app.get("/stats", async (c2) => {
  try {
    const vipData = await getVIPList(c2.env);
    const stats = {
      totalVips: vipData.vips.length,
      avgAttempts: vipData.vips.length > 0 ? (vipData.vips.reduce((sum, v2) => sum + (v2.attempts || 1), 0) / vipData.vips.length).toFixed(1) : 0,
      methodBreakdown: vipData.vips.reduce((acc, v2) => {
        acc[v2.method || "unknown"] = (acc[v2.method || "unknown"] || 0) + 1;
        return acc;
      }, {})
    };
